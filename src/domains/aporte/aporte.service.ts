...
export const aporteService = {
    ...
    handleStripeWebhook: async (
        ...
        if (event.type === 'payment_intent.succeeded') {
            ...
            if (serialNumber) {
                await aporteRepository.updateSerialNumber(novoAporte.codAporte, serialNumber)
            }
        ...
        return aportes.map(aporte => {
            ...
            return {
                ...
                blockchain: registro ? {
                    ...
                    serialNumber: (aporte as any).serialNumber // Add Serial to List View
                } : { registrado: false }
            }
        })
    },
    ...
    getRastreabilidade: async (codAporte: number) => {
        ...
        return {
            ...
            serialNumber: (aporte as any).serialNumber,
                ...
                projeto: {
                    ...
                    pagamentos: aloc.projeto.execucao_marco.flatMap(em =>
                        em.pagto_marco_concluido.map(pagto => ({
                            ...
                            explorerUrlPagamento: pagto.transacoes[0]?.explorerUrl
                                ?? (pagto.transacoes[0]?.hash?.includes('@')
                                    ? `https://hashscan.io/${env.HEDERA_NETWORK}/transaction/${pagto.transacoes[0]?.hash}`
                                    : `https://hashscan.io/${env.HEDERA_NETWORK}/topic/${env.HEDERA_TOPIC_ID}/message/${pagto.transacoes[0]?.hash}`),
                        }))
                    )
                }
            }))
