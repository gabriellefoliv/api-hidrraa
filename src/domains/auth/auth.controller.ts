...
export const cadastrarEntDelTecHandler = async (request: any, reply: any) => {
    try {
        const { codUsuario } = await authService.cadastrarEntDelTec(request.body)
        return reply.status(201).send({ codUsuario })
    } catch (error: any) {
        if (error.message === 'Usuário já existe no sistema.') {
            return reply.status(409).send({ error: error.message })
...
export const cadastrarEntDelFinHandler = async (request: any, reply: any) => {
    try {
        const { codUsuario } = await authService.cadastrarEntDelFin(request.body)
        return reply.status(201).send({ codUsuario })
    } catch (error: any) {
        if (error.message === 'Usuário já existe no sistema.') {
            return reply.status(409).send({ error: error.message })
